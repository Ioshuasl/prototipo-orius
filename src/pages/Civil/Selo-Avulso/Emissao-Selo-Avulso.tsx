...
// --- Estado Inicial do Pedido de Selo Avulso ---
...
// --- Tipos e Opções para os Formulários ---
...
export default function EmissaoSeloAvulso() {
    const { protocolo } = useParams<{ protocolo: string }>(); // <-- 2. LER O PROTOCOLO DA URL
    const navigate = useNavigate(); // <-- 2. PARA REDIRECIONAR
    ...
    const [isEditing, setIsEditing] = useState(false); // <-- 2. ESTADO PARA CONTROLAR SE É EDIÇÃO

    // --- 3. LÓGICA PARA CARREGAR DADOS NA EDIÇÃO ---
    useEffect(() => {
        if (protocolo) {
            console.log("Modo de edição. Protocolo:", protocolo);
            ...
            if (seloExistente) {
                ...
                navigate('/selos/avulso'); // Redireciona para a página de gerenciamento
            }
        }
    }, [protocolo, navigate]);
    ...
    const pageTitle = isEditing ? "Editar Emissão de Selo Avulso" : "Emissão de Selo Avulso"; // <-- 3. TÍTULO DINÂMICO
        ...
        <div className="mx-auto space-y-6">
            <header className="mb-6">
                ...
                <p className="text-md text-gray-500 mt-1">
                    {isEditing ? `Editando o protocolo: ${pedido.protocolo}` : "Preencha os dados abaixo para solicitar a emissão de selos."}
                </p>
            </header>
            ...
            <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm space-y-8">
                ...
                <div className="p-4 bg-gray-50 rounded-lg border border-gray-300">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div className="md:col-span-2">
                            ...
                            <input type="text" name="nomePartePrincipal" value={pedido.referenciaAto.nomePartePrincipal} onChange={handleReferenciaAtoChange} className="mt-1 w-full border border-gray-300 rounded-md p-2 shadow-sm" />
                        </div>
                        <div>
                             <label className="block text-sm font-medium text-gray-700">Data do Registro</label>
                             <input type="date" name="dataRegistro" value={pedido.referenciaAto.dataRegistro} onChange={handleReferenciaAtoChange} className="mt-1 w-full border border-gray-300 rounded-md p-2 shadow-sm" />
                        </div>
                        <div className='md:col-span-1'>
                             <label className="block text-sm font-medium text-gray-700">Tipo de Ato</label>
                             <select name="tipoAto" value={pedido.referenciaAto.tipoAto} onChange={handleReferenciaAtoChange} className="mt-1 w-full border border-gray-300 rounded-md p-2 shadow-sm">
                ...
                <div className="p-4 bg-gray-50 rounded-lg border border-gray-300">
                     ...
                     <div className="space-y-4">
                        {pedido.selosSolicitados.map((item) => (
                            <div key={item.id} className="grid grid-cols-12 gap-4 items-start p-3 bg-white border rounded-md">
                                <div className="col-span-12 md:col-span-6">
                                    <label className="text-xs font-medium text-gray-500">Tipo de Selo/Ato</label>
                                    <SeloSearchInput
                                        selectedSeloId={item.id_selo}
                                        onSeloSelect={(selo) => handleSeloItemUpdate(item.id, selo)}
                                        onClear={() => handleSeloItemUpdate(item.id, null)}
                                    />
                                </div>
                                <div className="col-span-4 md:col-span-1">
                                    <label className="text-xs font-medium text-gray-500">Qtd.</label>
                                    <input type="number" min="1" value={item.quantidade} onChange={(e) => handleQuantidadeChange(item.id, parseInt(e.target.value))} className="mt-1 w-full border border-gray-300 rounded-md p-2" />
                                </div>
                                <div className="col-span-4 md:col-span-2 text-center pt-1">
                                    <p className="text-xs font-medium text-gray-500">Valor Unit.</p>
                                    <p className="font-semibold mt-2">{formatCurrency(item.valorUnitario)}</p>
                                </div>
                                 <div className="col-span-4 md:col-span-2 text-center pt-1">
                                    <p className="text-xs font-medium text-gray-500">Subtotal</p>
                                    <p className="font-bold text-blue-600 mt-2">{formatCurrency(item.valorTotal)}</p>
                                </div>
                                <div className="col-span-12 md:col-span-1 flex justify-end items-start pt-4">
                                     <button onClick={() => handleRemoveSelo(item.id)} className="p-2 text-red-500 hover:bg-red-100 rounded-full"><Trash2 size={18}/></button>
                                </div>
                            </div>
                        ))}
                        <button onClick={handleAddSelo} className="flex items-center gap-2 text-sm bg-blue-100 text-blue-800 font-semibold px-4 py-2 rounded-lg hover:bg-blue-200">
                ...
                <div className="text-right pt-6 flex items-center justify-end gap-4">
                    <button type="button" onClick={() => navigate('/selos/avulso')} className="bg-gray-200 text-gray-800 font-semibold px-6 py-3 rounded-lg hover:bg-gray-300">
                        Cancelar
                    </button>
                    <button onClick={handleAvancar} className="bg-green-600 text-white font-semibold px-6 py-3 rounded-lg shadow-md hover:bg-green-700 flex items-center gap-2">
