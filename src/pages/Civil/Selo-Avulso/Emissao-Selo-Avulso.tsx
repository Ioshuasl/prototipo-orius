...
// Essa lista é mantida para o cálculo de valores, mas a busca agora está no SeloSearchInput
...
// --- O componente SeloSearchInput FOI REMOVIDO DAQUI ---
...
export default function EmissaoSeloAvulso() {
    ...
    // Lógica para recalcular os totais
    useEffect(() => {
        const total = pedido.selosSolicitados.reduce((acc, item) => acc + item.valorTotal, 0);
        
        // Recalcular emolumentos e taxas a partir da TabelaEmolumentos
    ...
    // A função é chamada pelo SeloSearchInput para atualizar o item
        ...
        const novosItens = pedido.selosSolicitados.map(item => {
            if (item.id === id) {
                if (seloInfo) {
                    ...
                    return { 
                        ...item, 
                        id_selo: seloInfo.id_selo, 
                        descricao: seloInfo.descricao_selo, 
                        valorUnitario, 
                        valorTotal: item.quantidade * valorUnitario // Recalcula o total
                    };
                } else {
                        ...
                        {pedido.selosSolicitados.map((item) => (
                            <div key={item.id} className="grid grid-cols-12 gap-4 items-start p-3 bg-white border border-gray-300 rounded-md">
                                <div className="col-span-12 md:col-span-6">
                                    ...
                                    {/* Componente SeloSearchInput Reutilizado */}
                                    <SeloSearchInput 
                                ...
                                <div className="col-span-4 md:col-span-1">
                                    ...
                                    <input type="number" min="1" value={item.quantidade} onChange={(e) => handleQuantidadeChange(item.id, parseInt(e.target.value))} className={commonInputClass} />
                                </div>
