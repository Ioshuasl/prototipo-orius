...
const mockLivros: Livro[] = Array.from({ length: 25 }, (_, i) => {
    ...
    return {
        ...
        folhaAtual: Math.floor(Math.random() * 200) + 1,
        folhasTotais: 200,
    };
});
...
export default function GerenciamentoLivrosPage() {
    ...
    const handleFecharLivro = (id: number) => {
        ...
        setFilteredRecords(prevRecords =>
            prevRecords.map(livro =>
                livro.id === id ? { ...livro, status: 'Fechado', dataFechamento: dataFechamentoFormatada } : livro
            )
        );
    };
    ...
    const LivroCard = ({ record }: { record: Livro }) => {
        return (
            // ALTERADO: Adicionado 'group' para habilitar o hover no título
            <Link
                to={`${record.id}`}
                className={`group block bg-white rounded-lg border border-gray-200 transition-all duration-300 hover:shadow-lg overflow-hidden`}
            >
                <div className="p-4">
                    <div className="flex justify-between items-start gap-4">
                        <div>
                             {/* ALTERADO: Adicionado efeito de hover no título */}
                            <h3 className="font-bold text-gray-800 text-lg leading-tight transition-colors">{`${record.numero} - Livro de ${record.tipo}`}</h3>
                        </div>
                        <StatusBadge status={record.status} />
                    </div>
                </div>
                <div className="p-4 grid grid-cols-2 gap-4 text-sm bg-gray-50/50 border-t border-b border-gray-100">
                    <div><p className="text-xs text-gray-500">Data de Abertura</p><p className="font-medium text-gray-700">{record.dataAbertura}</p></div>
                    <div><p className="text-xs text-gray-500">Data de Fechamento</p><p className="font-medium text-gray-700">{record.dataFechamento || '—'}</p></div>
                    <div><p className="text-xs text-gray-500">Páginas</p><p className="font-medium text-gray-700">{`${record.folhaAtual} / ${record.folhasTotais} (${((record.folhaAtual / record.folhasTotais) * 100).toFixed(1)}%)`}</p></div>
                </div>
                <div className="p-2 flex justify-end items-center gap-2">
                    <button onClick={(e) => { e.preventDefault(); window.print(); }} className="flex items-center gap-2 text-sm bg-gray-50 text-gray-700 font-semibold px-3 py-1.5 rounded-md hover:bg-gray-200 transition-colors">
                        <PrinterIcon size={14} /> Imprimir Livro
                    </button>
                    {record.status === 'Aberto' && (
                        <button onClick={(e) => { e.preventDefault(); handleFecharLivro(record.id); }} className="flex items-center gap-2 text-sm bg-red-50 text-red-700 font-semibold px-3 py-1.5 rounded-md hover:bg-red-100 transition-colors">
    ...
    return (
        <>
            <title>Gerenciamento de Livros | Orius Tecnologia</title>
            <div className="mx-auto space-y-4 pb-12">
                <header className="flex items-center justify-between">
                    {/* ALTERADO: Cor do título e do botão principal */}
                    <div><h1 className="text-3xl font-bold text-[#4a4e51]">Gerenciamento de Livros</h1><p className="text-md text-gray-500 mt-1">Consulte e gerencie os livros do cartório.</p></div>
                        ...
                        <div className="overflow-hidden">
                            <div className="p-5 space-y-4">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                    <div><label htmlFor="tipo" className="block text-sm font-medium text-gray-600 mb-1">Tipo de Livro</label><select id="tipo" name="tipo" value={filters.tipo} onChange={handleFilterChange} className="border border-gray-300 rounded-md px-3 py-2 w-full focus:ring-2 focus:ring-[#dd6825]/50 focus:border-[#dd6825]"><option>Todos</option>{tipoOptions.map(s => <option key={s}>{s}</option>)}</select></div>
                                    ...
                                    <div><label htmlFor="numero" className="block text-sm font-medium text-gray-600 mb-1">Número do Livro</label><input id="numero" type="text" name="numero" value={filters.numero} onChange={handleFilterChange} placeholder="Ex: 101" className="border border-gray-300 rounded-md px-3 py-2 w-full focus:ring-2 focus:ring-[#dd6825]/50 focus:border-[#dd6825]" /></div>
                                </div>
