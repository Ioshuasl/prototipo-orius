...
export default function ModalConfiguracaoImpressao({ isOpen, onClose, onSave, configuracaoAtual }: ModalProps) {
    ...
    // ALTERADO: Centralização da classe de foco para inputs
        ...
        <div className="fixed inset-0 z-50 flex justify-center items-center p-4 animate-fade-in-fast bg-gray-900/50 backdrop-blur-sm" aria-modal="true" role="dialog">
            <div className="bg-white rounded-lg shadow-xl w-full max-w-lg p-6 space-y-6">
                <div className="flex justify-between items-center">
                    ...
                    <h2 className="text-xl font-bold text-[#4a4e51]">Configurações de Impressão</h2>
                ...
                <fieldset className="border border-gray-200 rounded-lg p-4">
                    ...
                    <div className="mt-2">
                        ...
                        <select
                            ...
                            className={`w-full border border-gray-300 rounded-md px-3 py-2 text-sm bg-white focus:outline-none ${commonFocusStyle}`}
                        >
                ...
                <fieldset className="border border-gray-200 rounded-lg p-4">
                    ...
                    <div className="grid grid-cols-4 gap-4 mt-2">
                        {/* ALTERADO: Estilo de foco dos inputs de margem */}
                        <div>
                            <label htmlFor="top" className="block text-xs text-gray-600 mb-1">Superior</label>
                            <input type="number" id="top" name="top" value={tempConfig.margens.top} onChange={handleMarginChange} className={`w-full border border-gray-300 rounded-md px-2 py-1.5 text-sm ${commonFocusStyle}`} step="0.1" />
                        </div>
                         <div>
                            <label htmlFor="bottom" className="block text-xs text-gray-600 mb-1">Inferior</label>
                            <input type="number" id="bottom" name="bottom" value={tempConfig.margens.bottom} onChange={handleMarginChange} className={`w-full border border-gray-300 rounded-md px-2 py-1.5 text-sm ${commonFocusStyle}`} step="0.1" />
                        </div>
                         <div>
                            <label htmlFor="left" className="block text-xs text-gray-600 mb-1">Esquerda</label>
                            <input type="number" id="left" name="left" value={tempConfig.margens.left} onChange={handleMarginChange} className={`w-full border border-gray-300 rounded-md px-2 py-1.5 text-sm ${commonFocusStyle}`} step="0.1" />
                        </div>
                         <div>
                            <label htmlFor="right" className="block text-xs text-gray-600 mb-1">Direita</label>
                            <input type="number" id="right" name="right" value={tempConfig.margens.right} onChange={handleMarginChange} className={`w-full border border-gray-300 rounded-md px-2 py-1.5 text-sm ${commonFocusStyle}`} step="0.1" />
                        </div>
                    </div>
                </fieldset>
