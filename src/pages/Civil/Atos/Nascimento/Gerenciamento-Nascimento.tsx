...
const mockBirthRecords: BirthRecord[] = Array.from({ length: 28 }, (_, i) => {
    ...
    const lavraturaDate = new Date(registrationDate.getTime() + (2 * 24 * 60 * 60 * 1000));

    return {
        id: i + 1,
        protocol: `2025-N-${String(12345 + i).padStart(5, '0')}`,
        name: `Criança Exemplo Feliz ${i + 1}`,
...
export default function BirthRecordsManagementPage() {
    ...
    const [filteredRecords, setFilteredRecords] = useState<BirthRecord[]>(mockBirthRecords);
    ...
    useEffect(() => {
        ...
        const filterTimeout = setTimeout(() => {
            const results = mockBirthRecords.filter(record => {
                const searchTermMatch = filters.searchTerm ? record.name.toLowerCase().includes(filters.searchTerm.toLowerCase()) || record.protocol.includes(filters.searchTerm) || record.cpf.includes(filters.searchTerm) : true;
                ...
                return searchTermMatch && statusMatch && isOldBookMatch && livroMatch && regStartDateMatch && regEndDateMatch && lavraturaStartDateMatch && lavraturaEndDateMatch;
            });
                    ...
                    <div className="mx-auto space-y-2">
                        ...
                        <nav className="flex items-center justify-between pt-4">
                            <p className="text-sm text-gray-600">
                                Mostrando <span className="font-semibold">{paginatedRecords.length > 0 ? ((currentPage - 1) * recordsPerPage) + 1 : 0}</span> a <span className="font-semibold">{Math.min(currentPage * recordsPerPage, filteredRecords.length)}</span> de <span className="font-semibold">{filteredRecords.length}</span> registros
                            </p>
