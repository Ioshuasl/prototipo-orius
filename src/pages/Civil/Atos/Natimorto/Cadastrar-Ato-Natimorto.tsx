...
export default function CadastrarAtoNatimortoForm() {
    ...
    // ALTERADO: Estilos de foco e hover com as cores da marca.
    ...
    const InfoButton = ({ title, content }: { title: string, content: React.ReactNode }) => (
        <button type="button" onClick={() => openInfoModal(title, content)} className="ml-2 text-gray-400 hover:text-[#dd6825] transition-colors">
            <HelpCircle size={16} />
        </button>
    );
    ...
    return (
        <>
            <title>Registrar Ato de Natimorto | Orius Tecnologia</title>
                    ...
                    <div className="mx-auto">
                        <header className="mb-6 flex justify-between items-center">
                            <div>
                                ...
                                <h1 className="text-3xl font-bold text-[#4a4e51]">Registrar Novo Ato de Natimorto</h1>
                                <p className="text-md text-gray-500 mt-1">Livro C-Auxiliar - Assento de Natimorto</p>
                            </div>
                             {/* ALTERADO: Estilo de foco do botão */}
                            <button type="button" onClick={() => setIsHistoryModalOpen(true)} className="flex items-center gap-2 bg-white text-gray-600 font-semibold px-4 py-2 rounded-lg border border-gray-300 shadow-sm hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#dd6825]">
                            ...
                            <div className="tab-content bg-white p-6 rounded-xl border border-gray-200 shadow-sm min-h-[500px]">
                                ...
                                {activeTab === 'natimorto' && (
                                    <fieldset>
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-4 gap-x-6">
                                            ...
                                            <div className="md:col-span-4 pt-2"><SubSectionTitle>Informações do Natimorto</SubSectionTitle></div>
                                            ...
                                            <div className="md:col-span-3 grid grid-cols-2 gap-x-6">
                                                ...
                                                <div className="flex items-center justify-center self-end mb-2">
                                                    <input id="natimorto.nomeAtribuido" type="checkbox" name="natimorto.nomeAtribuido" checked={formData.natimorto.nomeAtribuido} onChange={handleInputChange} className="h-4 w-4 text-[#dd6825]" />
                                                    <label htmlFor="natimorto.nomeAtribuido" className="ml-2 text-sm font-medium flex items-center">Foi atribuído nome?<InfoButton title="Atribuição de Nome ao Natimorto" content="Diferente do registro de nascimento, a atribuição de um nome (prenome e sobrenome) ao natimorto é uma faculdade dos pais, não uma obrigação. O oficial deve indagar sobre essa opção." /></label>
                                                </div>
                                                <div className="flex items-center justify-center self-end mb-2"><input id="natimorto.partoDuplo" type="checkbox" name="natimorto.partoDuplo" checked={formData.natimorto.partoDuplo} onChange={handleInputChange} className="h-4 w-4 text-[#dd6825]" /><label htmlFor="natimorto.partoDuplo" className="ml-2 text-sm font-medium">Houve parto duplo?</label></div>
                                            </div>
                                            ...
                                            <div className="md:col-span-4 pt-2"><SubSectionTitle>Atestado Médico</SubSectionTitle></div>
                                            ...
                                            <div>
                                                <label htmlFor="natimorto.numeroDO" className={`${commonLabelClass} flex items-center`}>Nº da Declaração de Óbito (DO) {requiredSpan}<InfoButton title="Documento Base para o Registro" content="Para o registro de natimorto, é obrigatória a apresentação da Declaração de Óbito (DO), e não da Declaração de Nascido Vivo (DNV). O número da DO deve ser inserido no registro." /></label>
                                ...
                                {activeTab === 'filiacao' && (
                                    <fieldset className="space-y-12">
                                        <div><SectionTitle>Dados da Mãe</SectionTitle><PersonFields personData={formData.filiacao.mae} pathPrefix={['filiacao', 'mae']} handleInputChange={handleInputChange} handleAddressUpdate={handleAddressUpdate} handleCpfSearch={handleCpfSearch} searchingCpf={searchingCpf} /></div>
                                        <div><SectionTitle>Dados do Pai</SectionTitle><PersonFields personData={formData.filiacao.pai} pathPrefix={['filiacao', 'pai']} handleInputChange={handleInputChange} handleAddressUpdate={handleAddressUpdate} handleCpfSearch={handleCpfSearch} searchingCpf={searchingCpf} /></div>
                                    </fieldset>
                                )}

                                {activeTab === 'testemunhas' && (
                                    <fieldset>
                                        <div className='flex items-center mb-6 pb-3 border-b border-gray-200'><h3 className="text-xl font-semibold text-gray-800">Dados das Testemunhas</h3><InfoButton title="Obrigatoriedade das Testemunhas" content="O Art. 706, inciso VI, exige a presença e a qualificação completa de duas testemunhas para a lavratura do assento de natimorto." /></div>
                                            ...
                                            {formData.testemunhas.map((witness, index) => (
                                                <div key={index} className="border-t first:border-t-0 pt-8 first:pt-0">
                                                    <div className="flex justify-between items-center mb-4">
                                                        ...
                                                        {formData.testemunhas.length > 2 && (<button type="button" onClick={() => handleRemoveTestemunha(index)} className="flex items-center gap-2 text-sm text-red-600 hover:text-red-800 font-semibold"><Trash2 className="h-4 w-4" />Remover Testemunha</button>)}
                                                    </div>
                                                    <PersonFields personData={witness} pathPrefix={['testemunhas', index]} handleInputChange={handleInputChange} handleAddressUpdate={handleAddressUpdate} handleCpfSearch={handleCpfSearch} searchingCpf={searchingCpf} />
                                                </div>
                                            ))}
                                        </div>
                                        <div className="mt-6 flex justify-start pt-6 border-t"><button type="button" onClick={handleAddTestemunha} className="flex items-center gap-2 bg-gray-100 text-gray-800 font-semibold px-4 py-2 rounded-lg shadow-sm border border-gray-300 hover:bg-gray-200 transition-colors"><PlusCircle className="h-5 w-5" />Adicionar Outra Testemunha</button></div>
                                    </fieldset>
                                )}
                                    ...
                                    <fieldset>
                                        ...
                                        <div className="space-y-4">
                                            {formData.documentosApresentados.map((doc, index) => (
                                                <div key={index} className="flex flex-col md:flex-row items-start gap-4 p-4 rounded-lg border border-gray-300 shadow-sm">
                                                    ...
                                                    <div className="w-full md:w-auto">
                                                        ...
                                                        <label htmlFor={`doc-file-${index}`} className="mt-1 cursor-pointer flex items-center justify-center gap-2 w-full md:w-56 px-4 py-2 bg-white text-[#dd6825] border border-gray-300 rounded-md shadow-sm hover:bg-gray-100 transition"><UploadCloud size={18} /><span>{doc.nomeArquivo ? 'Trocar Arquivo' : 'Escolher Arquivo'}</span></label><input id={`doc-file-${index}`} type="file" className="sr-only" onChange={(e) => handleFileChange(['documentosApresentados', index], e.target.files?.[0] || null)} />{doc.nomeArquivo && (<p className="text-xs text-green-700 mt-1 truncate w-full md:w-56" title={doc.nomeArquivo}>Arquivo: {doc.nomeArquivo}</p>)}
                                                    </div>
                                        ...
                                        <button type="button" onClick={handleAddDocumento} className="mt-6 flex items-center gap-2 text-sm font-medium text-[#dd6825] hover:text-[#c25a1f]"><PlusCircle size={16} /> Adicionar Documento</button>
                                    </fieldset>
                                )}
                            </div>
            ...
            <InfoModal isOpen={modalContent !== null} onClose={closeInfoModal} title={modalContent?.title || ''}>
