...
const mockStillbornRecords: StillbornRecord[] = Array.from({ length: 14 }, (_, i) => {
    ...
    const dateOfOccurrence = new Date(registrationDate.getTime() - (2 * 24 * 60 * 60 * 1000));
    const lavraturaDate = new Date(registrationDate.getTime() + (1 * 24 * 60 * 60 * 1000));

    return {
        id: i + 1,
        protocol: `2025-Nati-${String(11223 + i).padStart(5, '0')}`,
        motherName: `Mãe de Exemplo ${i + 1}`,
        motherCpf: `444.555.666-${String(10 + i).padStart(2, '0')}`,
        gestationalAge: `${20 + i} semanas`,
        dateOfOccurrence: dateOfOccurrence.toLocaleDateString('pt-BR'),
...
export default function StillbornRecordsManagementPage() {
    ...
    const [filteredRecords, setFilteredRecords] = useState<StillbornRecord[]>(mockStillbornRecords);
    ...
    useEffect(() => {
        ...
        const filterTimeout = setTimeout(() => {
            const results = mockStillbornRecords.filter(record => {
                const searchTermMatch = filters.searchTerm ? record.motherName.toLowerCase().includes(filters.searchTerm.toLowerCase()) || record.protocol.includes(filters.searchTerm) || record.motherCpf.includes(filters.searchTerm) : true;
                ...
                const dateOfOccurrence = parseDate(record.dateOfOccurrence);
                const occurrenceStartDateMatch = filters.occurrenceStartDate && dateOfOccurrence ? dateOfOccurrence >= new Date(filters.occurrenceStartDate + 'T00:00:00') : true;
                const occurrenceEndDateMatch = filters.occurrenceEndDate && dateOfOccurrence ? dateOfOccurrence <= new Date(filters.occurrenceEndDate + 'T23:59:59') : true;
                return searchTermMatch && statusMatch && isOldBookMatch && livroMatch && regStartDateMatch && regEndDateMatch && lavraturaStartDateMatch && lavraturaEndDateMatch && occurrenceStartDateMatch && occurrenceEndDateMatch;
            });
