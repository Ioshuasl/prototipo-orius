...
const UserCard = ({ usuario, getCargoNome, onDelete }: {
    ...
    return (
        ...
        <div className="bg-white rounded-lg border border-gray-200 shadow-sm p-5 flex flex-col transition-all hover:shadow-lg hover:border-[#dd6825]/50">
            <Link to={`${usuario.id}`}>
            ...
            <div className="mt-6 pt-4 flex justify-end gap-3">
                {/* ALTERADO: Cores do hover no botão de editar */}
                <Link to={`${usuario.id}`} className="p-2 text-gray-500 hover:text-[#dd6825] hover:bg-[#dd6825]/10 rounded-full transition-colors">
                    <Edit size={20} />
                </Link>
...
const GerenciamentoUsuarios: React.FC = () => {
    ...
    // ALTERADO: Estilo de foco centralizado para inputs
    const commonInputClass = "w-full border border-gray-300 rounded-md p-2 shadow-sm focus:ring-2 focus:ring-[#dd6825]/50 focus:border-[#dd6825]";

    return (
        <>
            <title>Usuários do Sistema | Orius Tecnologia</title>
            <div className="mx-auto">
                <header className="flex items-center justify-between mb-6 pb-4">
                    <div>
                        {/* ALTERADO: Cor do título principal */}
                        <h1 className="text-3xl font-bold text-[#4a4e51]">Usuários do Sistema</h1>
                    ...
                    <Link to="cadastrar" className="flex items-center gap-2 bg-[#dd6825] text-white font-semibold px-4 py-2 rounded-lg shadow-sm hover:bg-[#c25a1f] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#dd6825]">
