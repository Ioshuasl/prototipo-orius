// Salve como src/pages/GerenciamentoRecibos.tsx
...
const ReciboCard = ({ template, onDelete }: {
    ...
    return (
        <div className="bg-white rounded-lg border border-gray-200 shadow-sm flex flex-col transition-all hover:shadow-xl hover:border-blue-300">
            ...
            <div className="p-4 border-t bg-gray-50/50 flex justify-end items-center gap-3">
                <Link to={`${template.id}`} className="p-2 text-gray-500 hover:text-blue-600 hover:bg-blue-50 rounded-full" aria-label={`Editar ${template.titulo}`}>
                    <Edit size={20} />
                </Link>
    ...
    return (
        <div className="max-w-7xl mx-auto p-6">
            <header className="flex flex-col md:flex-row items-start md:items-center justify-between mb-6 pb-4 border-b">
                ...
                <Link to="/admin/recibos/novo" className="flex items-center gap-2 bg-blue-600 text-white font-semibold px-4 py-2 rounded-lg shadow-sm hover:bg-blue-700 mt-4 md:mt-0">
                    <PlusCircle className="h-5 w-5" />
                    Novo Modelo
                </Link>
            </header>

            {/* Abas para filtrar por Tipo de Recibo */}
            ...
            {filteredTemplates.length > 0 ? (
                ...
                <div className="text-center py-12 bg-gray-50 rounded-lg">
                    ...
                    <Link to="cadastrar" className="mt-4 inline-block text-blue-600 font-semibold hover:underline">
