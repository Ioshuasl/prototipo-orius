...
// ALTERADO: Estilos dos badges e cards agora usam a paleta de cores da marca.
const tipoPessoaStyles: Record<Exclude<TipoPessoa, 'Todos'>, { text: string, bg: string, border: string, topBorder: string, icon: React.ReactNode }> = {
    'Pessoa Física': { text: 'text-gray-800', bg: 'bg-gray-100', border: 'border-gray-200', topBorder: 'border-t-gray-500', icon: <User size={12} /> },
    'Pessoa Jurídica': { text: 'text-[#c25a1f]', bg: 'bg-[#dd6825]/10', border: 'border-[#dd6825]/20', topBorder: 'border-t-[#dd6825]', icon: <Building size={12} /> },
};
...
const formatarCpfCnpj = (valor: string): string => {
    ...
    let mascara = '';
    if (apenasDigitos.length <= 11) { mascara = '000.000.000-00'; } 
    else { mascara = '00.000.000/0000-00'; }
    const mask = IMask.createMask({ mask: mascara });
    mask.resolve(apenasDigitos);
    return mask.value;
};

export default function GerenciamentoPessoasPageNotas() {
    ...
    const PessoaCard = ({ record }: { record: TPessoaTipo }) => {
        ...
        const renderInfo = () => {
            ...
            if (record.tipo === 'fisica') {
                return ( <>
                    <h3 className="font-bold text-gray-800 text-base leading-tight transition-colors pr-8">{record.nome}</h3>
                    <p className="text-sm text-gray-500 truncate mt-1">CPF: {formatarCpfCnpj(record.cpf)}</p>
                </> );
            }
            return ( <>
                <h3 className="font-bold text-gray-800 text-base leading-tight transition-colors pr-8">{record.razaoSocial}</h3>
            ...
            <div className="relative">
                <Link to={`${recordId}`} className="block group">
                    <div className={`bg-white rounded-lg border border-gray-200 transition-all duration-300 hover:shadow-xl hover:border-gray-300 overflow-hidden`}>
                        <div className="p-4 border-b border-gray-100"><div className='mb-3'><TipoPessoaBadge tipo={record.tipo === 'fisica' ? 'Pessoa Física' : 'Pessoa Jurídica'} /></div>{renderInfo()}</div>
                        <div className="p-4 bg-gray-50/50">
                            <div className="flex items-start gap-3 text-sm">
                                <MapPin className="h-4 w-4 mt-0.5 text-gray-400 flex-shrink-0" />
                                <div><p className="font-medium text-gray-700">{`${record.endereco.logradouro}, ${record.endereco.numero}`}</p><p className="text-gray-500">{`${record.endereco.bairro} - ${record.endereco.cidade}, ${record.endereco.uf}`}</p></div>
                            </div>
                        </div>
                    </div>
                </Link>
    ...
    // ALTERADO: Estilo de foco nos inputs e selects
    const commonFocusStyle = "focus:ring-2 focus:ring-[#dd6825]/50 focus:border-[#dd6825]";

    return (
        <>
            <title>Gerenciamento de Pessoas | Orius Tecnologia</title>
            <div className="mx-auto space-y-6 pb-12">
                <header className="flex items-center justify-between">
                    {/* ALTERADO: Cor do título e do botão principal */}
                    <div><h1 className="text-3xl font-bold text-[#4a4e51]">Gerenciamento de Pessoas</h1><p className="text-md text-gray-500 mt-1">Consulte, cadastre e gerencie pessoas físicas e jurídicas.</p></div>
                    ...
                    <div className={`grid transition-all duration-500 ease-in-out ${filtersVisible ? 'grid-rows-[1fr] opacity-100' : 'grid-rows-[0fr] opacity-0'}`}>
                        <div className="overflow-hidden">
                            <div className="p-5 space-y-4">
                                <div><label htmlFor="searchTerm" className="block text-sm font-medium text-gray-600 mb-1">Buscar por Nome, Razão Social, CPF ou CNPJ</label><div className="relative"><Search className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-gray-400" /><input id="searchTerm" type="text" name="searchTerm" value={filters.searchTerm} onChange={handleFilterChange} placeholder="Digite para buscar..." className={`border border-gray-300 rounded-md pl-10 pr-4 py-2 w-full ${commonFocusStyle}`} /></div></div>
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                    <div><label htmlFor="tipoPessoa" className="block text-sm font-medium text-gray-600 mb-1">Tipo de Pessoa</label><select id="tipoPessoa" name="tipoPessoa" value={filters.tipoPessoa} onChange={handleFilterChange} className={`border border-gray-300 rounded-md px-3 py-2 w-full ${commonFocusStyle}`}>{tipoPessoaOptions.map(s => <option key={s}>{s}</option>)}</select></div>
                                    <div><label htmlFor="uf" className="block text-sm font-medium text-gray-600 mb-1">UF do Endereço</label><select id="uf" name="uf" value={filters.uf} onChange={handleFilterChange} className={`border border-gray-300 rounded-md px-3 py-2 w-full ${commonFocusStyle}`}><option>Todos</option>{ufs.map(uf => <option key={uf}>{uf}</option>)}</select></div>
                                    <div><label htmlFor="cidade" className="block text-sm font-medium text-gray-600 mb-1">Cidade do Endereço</label><input id="cidade" type="text" name="cidade" value={filters.cidade} onChange={handleFilterChange} placeholder="Digite o nome da cidade..." className={`border border-gray-300 rounded-md px-4 py-2 w-full ${commonFocusStyle}`} /></div>
                                </div>
                ...
                <div className="min-h-[400px] relative">
                    ...
                    {isLoading ? (
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">{paginatedRecords.map(record => (<PessoaCard key={record.tipo === 'fisica' ? record.cpf : record.cnpj} record={record} />))}</div>
                    ) : (
                        <div className="flex flex-col items-center justify-center text-center text-gray-500 bg-white rounded-xl h-full border-2 border-dashed py-10"><ListX className="h-12 w-12 mb-4" /><h3 className="text-lg font-semibold">Nenhuma pessoa encontrada</h3><p>Tente ajustar ou limpar os filtros para ver mais resultados.</p></div>
                    )}
                </div>
